...
__global__ void gelu_kernel(const float* x, float* out, int n) {
    ...
    if(idx < n){
        float v = x[idx];
        float v3 = v * v * v; 
        float scale = tanhf(sqrtf(2.0f / M_PI) * (v + 0.044715*v3));
        out[idx] = 0.5 * v * (1 + scale);
    }
}
...
__global__ void swiglu_kernel(const float* gate, const float* up, float* out, int n) {
    ...
    if(idx < n){
        float score = gate[idx] / (1 + expf(-gate[idx]));
        out[idx] = score * up[idx];
    }
}
...
__global__ void fused_add_rmsnorm_kernel(
    ...
    float local_sum=0.0f;
    for(int idx=threadIdx.x; idx < n; idx += blockDim.x){
        residual_out_row[idx] = x_row[idx] + residual_row[idx];
        local_sum += residual_out_row[idx] * residual_out_row[idx];
    }

    __shared__ float rms;
    float block_sum = block_reduce_sum(local_sum, smem);
    if (threadIdx.x == 0){
        rms = sqrtf(block_sum / n + eps);
    }
    __syncthreads();
    for(int idx = threadIdx.x; idx < n; idx += blockDim.x){
        out_row[idx] = (residual_out_row[idx] / rms) * weight[idx];
    }
...
__global__ void causal_softmax_kernel(const float* x, float* out, int rows, int cols) {
    ...
    for(int idx = threadIdx.x; idx < cols; idx += blockDim.x){
        float val = (idx > row) ? -INFINITY:x_row[idx];
        local_max = fmaxf(local_max, val);
    }

    float blockMax = block_reduce_max(local_max, s_max);
    if(threadIdx.x == 0){
        row_max = blockMax; 
    }
    __syncthreads();
    ...
    float blockSum = block_reduce_sum(local_sum, s_sum);
    if(threadIdx.x==0){
        row_exp_sum=blockSum;
    }
    __syncthreads();
    for(int idx = threadIdx.x; idx < cols; idx += blockDim.x){
        float val = (idx > row) ? -INFINITY:x_row[idx];
        out_row[idx] = expf(val - row_max) / row_exp_sum;
    }
...
__global__ void embedding_lookup_kernel(const int* token_ids, const float* weight, float* out, int seq_len, int vocab_size, int embed_dim) {
    ...
    int total_size = seq_len * embed_dim;
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int token_pos = idx / embed_dim;
    int dim = idx % embed_dim;
    if(idx < total_size){
        out[idx] = weight[token_ids[token_pos] * embed_dim + dim];
    }
}
...
void rmsnorm_residual_block(
    ...
    int num_threads = 256;
    int num_blocks = rows;
    fused_add_rmsnorm_kernel<<<num_blocks, num_threads>>>(x, residual, weight, out, residual_out, n, eps);
}
